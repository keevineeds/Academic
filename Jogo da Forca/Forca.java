    ...
    public static void main (String args [])
    {
        ...
        do
        {
            try
            {
                ...
                new ControladorDeErros ((int)(palavra.getTamanho()*0.4));

                
                // revelar dica
                char letraDica = palavra.revelarLetraDica();
                int qtdDica = palavra.getQuantidade(letraDica);

                if(qtdDica > 1){
                    for(int i = 0 ; i < qtdDica; i++){
                        int pos = palavra.getPosicaoDaIezimaOcorrencia(i, letraDica);
                        tracinhos.revele(pos, letraDica);
                    }
                }else{
                    int pos = palavra.getPosicaoDaIezimaOcorrencia(qtdDica, letraDica);
                    tracinhos.revele(pos, letraDica);
                }
                controladorDeLetrasJaDigitadas.registre(letraDica);
